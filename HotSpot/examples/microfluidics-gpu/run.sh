...
#../../hotspot -c example.config -init_file outputs/${WORKLOAD}.init -p ptraces/${WORKLOAD}.ptrace -materials_file example.materials -grid_layer_file example.lcf -model_type grid -detailed_3D on -use_microchannels 1 -o outputs/${WORKLOAD}.ttrace -grid_transient_file outputs/${WORKLOAD}.grid.ttrace -grid_steady_file outputs/${WORKLOAD}.grid.steady
