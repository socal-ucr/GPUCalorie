						   ...
						   int n_orients)
{
	...
	/* rotable but no overlap, hence two sets of orientations	*/		
		...
		/* orientations with aspect ratios > 1	*/
		tmin = MAX2(min, 1.0/min);
		tmax = MAX2(max, 1.0/max);
		minx = sqrt(area * MIN(tmin, tmax));
		maxx = sqrt(area * MAX2(tmin, tmax));
		if ( n > 1)
			r = pow((maxx / minx) , 1.0/(n-1));
...
shape_t *shape_add(shape_t *shape1, shape_t *shape2, int cut_type)
{
	...
	while(i < m && j < n) {
		...
		if (cut_type == CUT_VERTICAL) {
			...
			sum->y[k] = MAX2(shape1->y[i], shape2->y[j]);
			sum->left_pos[k] = i;
			sum->right_pos[k] = j;
		...
		/* horizontal add */
			 ...
			 */
			sum->x[total-1-k] = MAX2(shape1->x[m-1-i], shape2->x[n-1-j]);
					   ...
					   flp_t *flp)
{
	...
	if (node->label.unit >= 0) {
		...
		if (node->label.unit == CUT_VERTICAL) {
			 ...
			 */
			if(self->y[pos] > MAX2(y1, y2)) {
				left->y[self->left_pos[pos]] += (self->y[pos] - MAX2(y1, y2));
				right->y[self->right_pos[pos]] += (self->y[pos] - MAX2(y1, y2));
				y1 = left->y[self->left_pos[pos]];
				y2 = right->y[self->right_pos[pos]];
			}	
			if(self->x[pos] > (x1+x2)) {
			...
			if(self->x[pos] > MAX2(x1, x2)) {
				left->x[self->left_pos[pos]] += (self->x[pos] - MAX2(x1, x2));
				right->x[self->right_pos[pos]] += (self->x[pos] - MAX2(x1, x2));
				x1 = left->x[self->left_pos[pos]];
				x2 = right->x[self->right_pos[pos]];
			}	
			if(self->y[pos] > (y1+y2)) {
